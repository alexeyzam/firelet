...
DAEMON_ARGS="--cf $CONF" # Arguments to run the daemon with
...
do_start()
{
	...
	start-stop-daemon --start --quiet --pidfile $PIDFILE --exec $DAEMON --test > /dev/null \
		|| return 1
	start-stop-daemon --start --quiet --pidfile $PIDFILE --chuid $DAEMONUSER --exec $DAEMON -- \
		$DAEMON_ARGS \
		|| return 2
}
...
do_stop()
{
	...
	start-stop-daemon --stop --quiet --retry=TERM/30/KILL/5 ---chuid $DAEMONUSER -pidfile $PIDFILE --name $NAME
	RETVAL="$?"
	[ "$RETVAL" = 2 ] && return 2
