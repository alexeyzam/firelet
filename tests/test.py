...
def test_parse_iptables_save_3(repodir):
    sx = MockSSHConnector(targets={'localhost':['127.0.0.1']})
...
def test_flssh_connect_one(mocked_connect, repodir):
...
def test_network_update(repodir):
...
def test_network_contains_networks(repodir):
...
def test_network_contains_hosts(repodir):
...
def test_host_contains_host(repodir):
    assert Host(['h', 'eth0', '1.1.1.1', 24, '1', '1', '1', [] ]) \
...
def test_bunch_hostgroup1(repodir):
    hg = HostGroup(['Servers'])
    assert hg.name == 'Servers'

def test_bunch_hostgroup2(repodir):
...
def test_flag_true(repodir):
    for x in (1, True, '1', 'True', 'y', 'on' ):
        assert flag(x) == '1'

def test_flag_false(repodir):
