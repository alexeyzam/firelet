...
from firelet.flcore import Rule, Host, Alert, HostGroup, Network, Users, validc
...
class TestConfReader(BaseFunctionalTesting):
...
@SkipTest
class TestMockSSHConnector(BaseFunctionalTesting):
    def test_MockSSHConnector_get_confs(self):
        sshconn = MockSSHConnector(targets={'localhost':['127.0.0.1']})
        sshconn.repodir = self._repodir
        d = sshconn.get_confs( )
...
#def test_get_confs3():
#    fs = DumbFireSet(repodir=self._repodir)
#    fs._get_confs()
...
class TestUserManagement(BaseFunctionalTesting):

    def test_user_management(self):
        u = Users(d=self._repodir)
...
class TestFileHandling(BaseFunctionalTesting):

    def test_load_save_hosts(self):
        fn = os.path.join(self._repodir, 'hosts.csv')
        content = map(str.strip, open(fn))
        content = filter(None, content)
        h = Hosts(d=self._repodir)
        h.save()
        content2 = map(str.strip, open(fn))
        content2 = filter(None, content2)
        h2 = Hosts(d=self._repodir)
    ...
    def test_load_save_csv(self):
        h = readcsv('rules', d=self._repodir)
        h = tuple(h)
        savecsv('rules', h, d=self._repodir)
        h2 = readcsv('rules', d=self._repodir)
        h2 = tuple(h2)
...
class TestFireSet(BaseFunctionalTesting):

    def test_gitfireset_otp(self):
        fs = GitFireSet(repodir=self._repodir)
        otp = fs.generate_otp()
        assert isinstance(otp, str)
        assert len(otp) == 10

    def test_gitfireset_simple(self):
        fs = GitFireSet(repodir=self._repodir)
    ...
    def test_gitfireset_smarttable_methods(self):
        fs = GitFireSet(repodir=self._repodir)
    ...
    def test_gitfireset_check_ifaces_1(self):
        fs = GitFireSet(repodir=self._repodir)
        fs._remote_confs = None
        assert_raises(AssertionError, fs._check_ifaces)

    def test_gitfireset_check_ifaces_20(self):
        fs = GitFireSet(repodir=self._repodir)
    ...
    def test_gitfireset_check_ifaces_wrong_value(self):
        fs = GitFireSet(repodir=self._repodir)
        fs._remote_confs = {'bogus': 'not a bunch'} # value should be a Bunch
        assert_raises(AssertionError, fs._check_ifaces)

    def test_gitfireset_check_ifaces_wrong_bunch_size(self):
        fs = GitFireSet(repodir=self._repodir)
        fs._remote_confs = {'bogus': Bunch()} # len(Bunch(...)) should be 2 (ip_addr_v4, ip_addr_v6)
        assert_raises(AssertionError, fs._check_ifaces)

    def test_gitfireset_check_ifaces_missing_iface(self):
        fs = GitFireSet(repodir=self._repodir)
        fs.hosts = [
    ...
    def test_gitfireset_check_ifaces_wrong_ipaddr_string(self):
    ...
    def test_gitfireset_check_ifaces_wrong_ipaddr(self):
    ...
    def test_gitfireset_check_ifaces_correct(self):
        fs = GitFireSet(repodir=self._repodir)
        fs.hosts = [
    ...
    def test_gitfireset_check_ifaces_correct2(self):
    ...
    def test_gitfireset_check_ifaces_alert(self):
    ...
    def test_gitfireset_sibling_names(self):
        fs = GitFireSet(repodir=self._repodir)
    ...
    def test_gitfireset_get_firewalls(self):
        fs = GitFireSet(repodir=self._repodir)
    ...
    def test_DemoGitFireSet_get_confs(self):
        fs = DemoGitFireSet(repodir=self._repodir)
    ...
    def test_DemoGitFireSet_deployment(self):
    ...
    def test_DemoGitFireSet_compile_rules_basic(self):
    ...
    def test_DemoGitFireSet_compile_rules_full(self):
        fs = GitFireSet(repodir=self._repodir)
        rd = fs.compile_rules()
        ok = {
            "InternalFW": {
    ...
    @SkipTest
    def test_DemoGitFireSet_build_ipt_restore(self):
    ...
    def test_DemoGitFireSet_extract_iptables_rules(self):
        fs = DemoGitFireSet(repodir=self._repodir)
    ...
    def test_DemoGitFireSet_extract_iptables_rules_2(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        remote_confs = {
            'InternalFW': {
                'iptables': {'filter': [], 'nat': []},
    ...
    def test_DemoGitFireSet_diff_table_generation_1(self):
        """Test diff with no changes"""
        fs = DemoGitFireSet(repodir=self._repodir)
        diff_dict = fs._diff({}, {})
        assert diff_dict == {}

    def test_DemoGitFireSet_diff_table_generation_2(self):
        """Test diff with no changes"""
        fs = DemoGitFireSet(repodir=self._repodir)
        diff_dict = fs._diff({'InternalFW':['']}, {'InternalFW':['']})
        assert diff_dict == {}

    def test_DemoGitFireSet_diff_table_generation_3(self):
        fs = DemoGitFireSet(repodir=self._repodir)
    ...
    def test_DemoGitFireSet_diff_table_generation_all_fw_removed(self):
    ...
    def test_DemoGitFireSet_diff_table_generation_all_fw_added(self):
    ...
    def test_DemoGitFireSet_check(self):
    ...
    def test_DemoGitFireSet_deploy(self):
        """Run diff between complied rules and remote confs.
        Given the test files, the check should be ok and require no deployment"""
        fs = DemoGitFireSet(repodir=self._repodir)
        log.debug("Test deployment in %s" % self._repodir)
        fs.deploy()
        for h in fs._get_firewalls():
            ok = open(self._repodir + '/iptables-save-%s' % h.hostname).readlines()
            r = open(self._repodir + '/iptables-save-%s-x' % h.hostname).readlines()
            assert ok == r
    ...
    def test_DemoGitFireSet_deploy_then_check(self):
        """Deploy conf then run check again"""
        fs = DemoGitFireSet(repodir=self._repodir)
        assert not fs.save_needed()
        log.debug("Running deployment using repository in %s" % self._repodir)
        fs.deploy()
        log.debug("Deployed.")
        assert not fs.save_needed()
    ...
    def test_DemoGitFireSet_service_update_error1(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update({})

    @raises(Alert)
    def test_DemoGitFireSet_service_update_incorrect_tcp_ports(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='TCP', ports='foo,foo'), rid=0)

    @raises(Alert)
    def test_DemoGitFireSet_service_update_missing_tcp_ports(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='TCP', ports=','), rid=0)

    @raises(AssertionError)
    def test_DemoGitFireSet_service_update_reversed_tcp_ports(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='TCP', ports='10:1'), rid=0)

    def test_DemoGitFireSet_service_update_tcp(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='TCP', ports='8888', name='HTTP'), rid=0)
        assert fs.services[0].ports == '8888'

    def test_DemoGitFireSet_service_update_ip(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='IP', ports='', name='IP'), rid=0)
        assert fs.services[0].ports == ''

    @raises(Alert)
    def test_DemoGitFireSet_service_update_incorrect_icmp_type(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='ICMP', ports='foo'), rid=0)

    @raises(Alert)
    def test_DemoGitFireSet_service_update_incorrect_protocol(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.update(dict(protocol='foo', ports=''), rid=0)

    def test_DemoGitFireSet_service_update_icmp(self):
        fs = DemoGitFireSet(repodir=self._repodir)
    ...
    def test_DemoGitFireSet_service_add(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.services.add(dict(protocol='ICMP', ports='8', name='NewName'))

    @raises(AssertionError)
    def test_DemoGitFireSet_service_add_duplicate(self):
        fs = DemoGitFireSet(repodir=self._repodir)
    ...
    @raises(AssertionError)
    def test_DemoGitFireSet_rules_update_missing_rid(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.rules.update({})

    @raises(Alert)
    def test_DemoGitFireSet_rules_update_missing_rule(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.rules.update({}, rid=1000)

    @raises(KeyError)
    def test_DemoGitFireSet_rules_update_missing_param(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.rules.update({}, rid=0)

    def test_DemoGitFireSet_rules_update(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            action='',
            desc='desc_foo',
    ...
    def test_DemoGitFireSet_rules_add(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            action='',
            desc='desc_foo',
    ...
    def test_DemoGitFireSet_rules_add_empty(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        fs.rules.add({}, rid=0)

    @raises(Alert)
    def test_DemoGitFireSet_rules_add_duplicate(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            action='',
            desc='desc_foo',
    ...
    def test_DemoGitFireSet_hosts_add(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            hostname='',
            iface='',
    ...
    @raises(AssertionError)
    def test_DemoGitFireSet_hosts_add_duplicate(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            hostname='InternalFW',
            iface='eth0',
    ...
    def test_DemoGitFireSet_hostgroups_add(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            name='',
            childs=[],
    ...
    @raises(AssertionError)
    def test_DemoGitFireSet_hostgroups_add_duplicate(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            name='AllSystems',
            childs=[],
    ...
    def test_DemoGitFireSet_hostgroups_update(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(name='foo', childs=[])
        fs.hostgroups.update(d, rid=0)

    @raises(Alert)
    def test_DemoGitFireSet_hostgroups_update_missing(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(name='foo', childs=[])
    ...
    def test_DemoGitFireSet_networks_add(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            name='',
            ip_addr='1.2.0.0',
    ...
    @raises(AssertionError)
    def test_DemoGitFireSet_networks_add_duplicate(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            name='Internet',
            ip_addr='1.2.0.0',
    ...
    @raises(Exception)
    def test_DemoGitFireSet_networks_add_incorrect_ipaddr(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            name='',
            ip_addr='foo',
    ...
    @raises(Exception)
    def test_DemoGitFireSet_networks_add_incorrect_netmask(self):
        fs = DemoGitFireSet(repodir=self._repodir)
        d = dict(
            name='',
            ip_addr='1.2.0.0',
...
@SkipTest  # FIXME
class TestMap(BaseFunctionalTesting):
...
class TestJson(BaseFunctionalTesting):
