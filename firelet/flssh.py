...
class MockSSHConnector(object):
    """Used in Demo mode and during unit testing."""
...
#                p.logout()
    ...
    def _interact(self, p, s):
        ...
        if s == 'sudo /sbin/iptables-save':
            return [x.rstrip() for x in open('/tmp/firelet/iptables-save-%s' % p).readlines()]
        elif s == '/bin/ip addr show':
            return [x.rstrip() for x in open('/tmp/firelet/ip-addr-show-%s' % p).readlines()]
    ...
    def deliver_confs(self, newconfs_d):
        ...
        for hostname, p in self._pool.iteritems():
            ...
            li.append('*filter')
            for iface, rules in newconfs_d[hostname].iteritems():
                [ li.append(str(rule)) for rule in rules ]
            li.append('COMMIT')
            block = '\n'.join(li)
            open('/tmp/firelet/iptables-save-%s' % p, 'w').write(block)
            ret = ''
            log.debug("Deployed ruleset file to %s, got %s" % (hostname, ret)  )
        return
