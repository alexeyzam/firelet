...
def compile(rules, hosts, hostgroups, services, networks):
    ...
    proto_port['*'] = (None, None) # special case for "any"
    ...
    for ena, name, src, src_serv, dst, dst_serv, action, log_val, desc in rules:
        ...
        sports = "-m multiport --sport %s" % sports if sports else ''
        dports = "-m multiport --sport %s" % dports if dports else ''

        log_val = int(log_val)  #TODO: try/except this
        if log_val:
            for src, dst in product(srcs, dsts):
                compiled.append("-A x %s -s %s %s -d %s %s -j LOG-%d" %   (proto, src, sports, dst, dports, log_val))

        for src, dst in product(srcs, dsts):
            compiled.append("-A x %s -s %s %s -d %s %s -j %s" %   (proto, src, sports, dst, dports, action))
